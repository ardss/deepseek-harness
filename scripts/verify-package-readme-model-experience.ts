...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/client/ui-workflow-artifacts': { kind: 'none', reason: 'Browser-side preset boards over already-logged run artifacts; registers nothing model-facing.' },
  'packages/client/ui-workflow-run-detail': { kind: 'none', reason: 'Browser-side run-detail presentation; renders durable records without changing model context.' },
  'packages/workflow/workflow-runs': { kind: 'none', reason: 'Pure run-state vocabulary and projection helpers; registers nothing model-facing.' },
  'packages/workflow/workflow-notifications': { kind: 'none', reason: 'Pure notification copy minting; the delivering tool owns every model-visible line.' },
  'packages/workflow/workflow-expert-core': { kind: 'none', reason: 'Engine kernel with no registrations of its own; the embedding runner and tools own every model-visible line.' },
